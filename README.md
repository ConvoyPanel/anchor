...
The `agent` role runs on a Proxmox VE node and bridges authenticated WebSocket sessions to local `qm vncproxy` or `qm terminal` processes. The optional `relay` role provides a shared public endpoint without exposing node addresses. The console frontend lives in Convoy.
